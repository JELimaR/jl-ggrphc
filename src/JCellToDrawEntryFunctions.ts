...
export const heigh = (alpha: number = 1) => {
	...
	return (c: JCell): IDrawEntry => {
		let color: string = colorScale(c.info.height).alpha(alpha).hex()
		return {
			fillColor: color,
			strokeColor: color
		}
...
export const heighLand = (alpha: number = 1) => {
	alpha = verifyAlpha(alpha);
	return (c: JCell): IDrawEntry => {
		let color: string = c.info.isLand ? colorScale(c.info.height).alpha(alpha).hex() : colorScale(0.05).alpha(alpha).hex();
		return {
			fillColor: color,
			strokeColor: color
		}
